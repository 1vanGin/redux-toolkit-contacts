...
(async () => {
  ...
  const contacts: ContactDto[] = contactValues
    .filter(({ status }) => status === "fulfilled")
    // @ts-ignore
    .map(({ value }) => value);

  const groupContactsValues = await genFull(
    () => genGroupContacts(contacts),
    20
  );

  const groupContacts: GroupContactsDto[] = groupContactsValues
    .filter(({ status }) => status === "fulfilled")
    // @ts-ignore
    .map(({ value }) => value);
  ...
  fs.writeFileSync(
    path.join(__dirname, pathToData, "groups-contacts.json"),
